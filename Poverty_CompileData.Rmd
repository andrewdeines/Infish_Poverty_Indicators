...
FAO_protein_2019<-read.csv("Data/protien supply_FAOSTAT_2010-2019.csv")#Load new data
...
FAO_protein <-
  rbind(FAO_protein_2013[, !names(FAO_protein_2013) %in% "Year.Code"], #bind data
        FAO_protein_2019%>%
          filter(Year >= 2014)%>%
          select(ï..Domain.Code,
                  Domain,
                  Area.Code..FAO.=Area.Code,
                  Area:Value,-Year.Code)) %>%
  group_by(Area, Year) %>%
...
# 
unique(FAO_protein[is.na(FAO_protein$ISO3.code),c("Country","Area.Code..FAO.")])
# Country                       Area.Code..FAO.
#1 Belgium-Luxembourg                         15
#2 China, mainland                            41
#3 Czechoslovakia                             51
#4 Ethiopia PDR                               62
#5 Netherlands Antilles (former)             151
#6 Serbia and Montenegro                     186
#7 Sudan (former)                            206
#8 USSR                                      228
#9 Yugoslav SFR                              248

```
There are currently 9 known problems with the country code matching, which aren't readily resolved.
`r unique(FAO_protein[is.na(FAO_protein$ISO3.code),c("Country","Area.Code..FAO.")])`
