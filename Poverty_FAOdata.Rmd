...
FAO_10yr_prod<-FAO_MPI%>%filter(Year%in%2009:2018)%>% #for ten-year average
 group_by(Country)%>%
 summarise(Prod_10yrAvg=mean(value_Total_t,na.rm = TRUE),
           frac_Capture_Avg=mean(frac_Capture,na.rm = TRUE),
           MPI=unique(MPI))

AMD01<-ggplot(data=FAO_10yr_prod,
       aes(x=Prod_10yrAvg,y=MPI,color=frac_Capture_Avg))+
  geom_point()+
  scale_y_log10()+
  scale_x_log10()+
  xlab("Total inland production 10 year average (capture+aquaculture t)")+
  scale_color_continuous("Avg Fraction Capture")+
  ggtitle("DRAFT- Country level MPI by total inland fisheries production")+
  ggrepel::geom_text_repel(data=FAO_10yr_prod[which(FAO_10yr_prod$frac_Capture_Avg>.95),],
...
AMD02<-ggplot(data=FAO_10yr_cons,
       aes(x=Cons_10yrAvg,y=MPI))+
  geom_point()+
  scale_y_log10()+
  scale_x_log10()+
  xlab("Inland fisheries consumption 10 year average")+
  ggtitle("DRAFT- Country level MPI by total inland fisheries consumption")
print(AMD02)

AMD03<-ggplot(data=full_join(FAO_10yr_prod,FAO_10yr_cons,by="Country"), 
              aes(x=Prod_10yrAvg,y=Cons_10yrAvg,color=frac_Capture_Avg))+
  geom_point()+
  ylab("10 year average consumption 2004-2013")+
  ylab("10 year average total production 2009-2018")+
  scale_color_continuous("Average Fraction Capture")+
  scale_x_log10()+
  scale_y_log10()
print(AMD03)
##Production and consumption are rather linear, so nothing interesting there. 
...
FAO_10yr<-FAO_MPI%>%
                ...
                summarise(x_10yr=median(value,na.rm = TRUE),
                          MPI=unique(MPI))

AMD05<-ggplot(data=FAO_10yr_mod,
              aes(x=MPI,y=x_10yr,color=name))+
  geom_point()+
  scale_y_log10()+
  scale_x_log10()+
  geom_smooth(method="lm")

FAO_10yr_lm_Capture<-lm(data=FAO_10yr,log(x_10yr+1)~log(MPI),subset=FAO_10yr$name%in%"value_Capture_t")
summary(FAO_10yr_lm_Capture)
plot(FAO_10yr_lm_Capture)

FAO_10yr_lm_Aquaculture<-lm(data=FAO_10yr,log(x_10yr+1)~log(MPI),subset=!FAO_10yr$name%in%"value_Capture_t")
summary(FAO_10yr_lm_Aquaculture)
plot(FAO_10yr_lm_Aquaculture)
